...
trait Lapi
{
    ...
    {
        // api设置
        $this->apiConfig = LapiConfigModel::getList();
        if (isset($this->apiConfig['api_close']) 
            && $this->apiConfig['api_close'] == 1
        ) {
            return $this->errorJson($this->apiConfig['api_close_tip'], 99999);
        }
        ...
        if (isset($this->apiConfig['open_putlog']) 
            && $this->apiConfig['open_putlog'] == 1
        ) {
            // 记录日志
            $this->createApiLog([
     ...
     * @author deatil
     */
    protected function checkUrlApiAuth()
    {
     ...
     * @author deatil
     */
    protected function createApiLog($data = []) 
    {
        if (empty($data)) {
            return false;
        }
     ...
     * @author deatil
     */
    protected function errorJson($msg = null, $code = 1) 
    {
        return $this->httpResponse(false, $code, $msg);
    }
     ...
     * @author deatil
     */
    private function httpResponse($success = true, $code, $msg = "", $data = [])
    {
